function [kl,ka,fd,nincs,sincs] = makeFault(ccx,ccy,ccr,maxarc,ps,slip,sinc)
...
% slip = fault slip, and sinc = fault slip increment,
%
% Output: kl = location of fault kinks, ka = angle of fault kinks with 
% the horizontal, fd = dip of fault segment, nincs = number of increments
% on fault segment, sincs = slip increment on fault segment
...
while carc <= maxarc
    ...
    % orientation of kink (parallel shear)
    ka(i-1)=(pi-(fd(i)-fd(i-1)))/2.0-fd(i-1);
...
for i=1:fs
    ...
    if rslip > 0.0
        ...
        if rslip < sfs || i == fs
            ...
            kl(i+1,1) = kl(i,1)+sfs*ps*cos(fd(i+1));
            kl(i+1,2) = kl(i,2)+sfs*ps*sin(fd(i+1));
        end
