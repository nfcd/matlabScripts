...
function chisq = backtrishear(xp,yp,zp,xtsf,ytsf,ztsf,xtnf,ytnf,ztnf,ramp,pss,psn,tras,tran,sls,sln,ninc,slrake) 

% Retrodeform bed for the griven model parameters
% and return sum of square of residuals (chisq), which
% is the objective function

% Pseudo 3D Trishear algorithm of Cristallini and Allmendinger
...
xp= fx*gp11+fy*gp21+fz*gp31+xtsf;
yp= fx*gp12+fy*gp22+fz*gp32+ytsf;
zp= fx*gp13+fy*gp23+fz*gp33+ztsf;

% Square of residuals 
XX = [ones(size(xp,2),1) xp' yp'];
YY = zp';
b = regress(YY,XX);
% objective function
chisq = sum((zp-b(1)-b(2)*xp-b(3)*yp).^2);
