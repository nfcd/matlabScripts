...
function [xpr,ypr,xpf,ypf] = restorebed(xp,yp,ccx,ccy,ccr,maxarc,ps,tra,slip,ashear,sinc) 

% Use this function to restore a bed to an initial detachment fold
% configuration and obtain a gaussian line (xpf,ypf) that best fits 
% the restored bed.
...
%(tra), slip, shear angle in backlimb (ashear), and slip increment (sinc)

% OUTPUT: restored bed (xpr,ypr)
% Best fit gaussian line to restored bed (xpf,ypf)
...
% Make fault geometry
[kl,ka,fd,R,nincs,sincs] = makeFault(ccx,ccy,ccr,maxarc,ps,slip,ashear,sincr);
fs = size(fd,1)-1;
    ...
    for i=1:nincs(ii)
        ...
        for j=1:size(xp,2)
            ...
            % If in footwall
                ...
                xx=fx-(psr*(i-1)*abs(sincs(ii))*R(ii+1));
                if xx >= 0.0 && fy >-xx*m
                    kinem = 1;
                end
            end
            ...
            if kinem == 0
                ...
                xp(j)=px+sincs(ii)*R(idk)*cos(fd(idk));
                yp(j)=py+sincs(ii)*R(idk)*sin(fd(idk));
            end
            % Trishear kinematics
            if kinem == 1
