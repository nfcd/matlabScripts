% Copyright: Nestor Cardozo 2012: Circular fault model and inclined shear
% in the backlimb

function [history] = gradcongen(xp,yp,ccx,ccy,ccr,maxarc,ps,tra,slip,ashear,sinc,maxit)
...
%(tra), slip, shear angle in backlimb (ashear), slip increment (sinc), 
...
% Uncomment line below if searching for slip, trishear angle, PS,  
% center and radius of curvature, and shear angle in backlimb
x0= [slip*1e-2 tra ps ccx*1e-2 ccy*1e-2 ccr*1e-2 ashear];
...
% Uncomment two lines below if searching for slip, trishear angle, 
% PS, center and radius of curvature and shear angle in backlimb
lb = [1.0 40.*pi/180. 1. 2.9 2.9 2.9 0.0];
ub = [3.0 80.*pi/180. 3. 3.1 3.1 3.1 60.0*pi/180.0];
...
simulannealbnd(@objfun,x0,lb,ub,options);
    ...
    function f = objfun(x)
        ...
        f = backtrishear(xp,yp,ccx,ccy,ccr,maxarc,ps,tra,slip,ashear,sinc);
    end
end