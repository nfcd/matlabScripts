...
% COMPUTE BEST FIT PARAMETERS USING GRADIENT BASED CONSTRAINED OPTIMIZATION
% MATLAB FUNCTION fmincon
% Trishear 3D algorithm of Cristallini et al. GSA Bulletin v. 116, 
% pp. 938-952 (2004)
...
options = optimset('OutputFcn',@outfuns,'Display','iter','MaxIter',maxit);
fmincon(@objfun,x0,[],[],[],[],lb,ub,[],options);

    function stop = outfuns(x,optimValues,state)
        stop = false;
        switch state
            case 'iter'
    ...
    function f = objfun(x)
        ...
        pss = x(5);
        psn = x(6);
         % Uncomment line below if searching for slip rake
        %slrake = x(7);
        % Uncomment line below if searching for ramp
        %ramp = x(8);
